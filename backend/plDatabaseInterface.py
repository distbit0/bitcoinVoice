...
#################################################################
def getFilteredPublicLabels(chainID, publicLabel, startDate, endDate, isUnSpent):
    ...
    # either return only unspent or return ALL
    ...
    if publicLabel :
        ...
        cursor.execute('SELECT * from "publicLabelOutput" where "chainID" = %s ' + isUnSpent + ' and "unixTimeCreated" >= %s and "unixTimeCreated" <= %s order by "txID", "txOutputSequence"', (chainID, float(startDate), float(endDate),))
...
def insertOrUpdateBlockInfoRecord(chainID, blockhash, unixTimeLastScan, countOutputsWithPublicLabels, countOutputsWithSpentPublicLabels, countOutputsWithErrors, txid, blockHeight) :
    ...
    cursor.execute('UPDATE "blockInfo" SET "lastErrorTxID" = %s, "unixTimeLastScan" = %s, "countOutputsWithPublicLabels" = %s, "countOutputsWithSpentPublicLabels" = %s, "countOutputsWithErrors" = %s, "blockHeight" = %s WHERE "chainID" = %s and "blockhash" = %s',
         (txid, unixTimeLastScan, countOutputsWithPublicLabels, countOutputsWithSpentPublicLabels, countOutputsWithErrors, str(blockHeight), chainID, blockhash))
